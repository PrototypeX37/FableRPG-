...
class Werewolf(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # channel_id -> "forming" | Game instance
        self.games: Dict[int, Union[str, Game]] = {}
    ...
    async def werewolf(
        ...
        speed: str = "Normal",
        min_players: Optional[IntGreaterThan(1)] = None,
    ):
        _(
            """
        ...
        game_modes = [
            ...
            "Villagergame",
            "Valentines",
            "Idlerpg",  # accepted alias for IdleRPG
            "IdleRPG",
        ]
        ...
        mode_str = str(mode) if mode is not None else "Classic"
        mode_str = (mode_str or "Classic").strip()
        speed = ((speed or "Normal").strip()).title()

        if mode_str not in game_modes:
            return await ctx.send(
                _(
                    "Invalid game mode. Use `{prefix}help ww` to get help on this command."
                ).format(prefix=ctx.clean_prefix)
            )

        if mode_str.lower() == "idlerpg":
            mode_str = "IdleRPG"

        if speed not in game_speeds:
            return await ctx.send(
                _(
                    "Invalid game speed. Use `{prefix}help ww` to get help on this command."
                ).format(prefix=ctx.clean_prefix)
            )

        if not min_players:
            min_players = minimum_players.get(mode_str, 5)

        self.games[ctx.channel.id] = "forming"

        additional_text = _(
            "Use `{prefix}help ww` to get help on werewolf commands. Use `{prefix}ww roles` to view descriptions of game roles and their goals to win. Use `{prefix}ww modes` and `{prefix}ww speeds` to see info about available game modes and speeds."
        ).format(prefix=ctx.clean_prefix)

        mode_emojis = {"Huntergame": "🔫", "Valentines": "💕"}
        mode_emoji = mode_emojis.get(mode_str, "")

        # Tournament / mass game channel
        if (
            getattr(self.bot.config.game, "official_tournament_channel_id", None)
            ...
            text = _(
                "**{author} started a mass-game of Werewolf!**\n**{mode}** mode on **{speed}** speed. You can join in the next 10 minutes. **Minimum of {min_players} players are required.**"
            )
            ...
            text = _(
                "**{author} started a game of Werewolf!**\n**{mode}** mode on **{speed}** speed. Minimum of **{min_players}** players are required. Starting in 2 minutes."
            )
            ...
            try:
                ...
                return await ctx.send(
                    _(
                        "An error happened during the Werewolf. Missing Permission: `Embed Links` . Please check the **Edit Channel > Permissions** and **Server Settings > Roles** then try again!"
                    )
                )
        ...
        if len(players) < int(min_players):
            self.games.pop(ctx.channel.id, None)
            await self.bot.reset_cooldown(ctx)
            return await ctx.send(
                _(
                    "Not enough players joined... We didn't reach the minimum {min_players} players. 🙁"
                ).format(min_players=min_players)
            )

        # IMPORTANT FIX: use stdlib shuffle (some forks have utils.random.shuffle = None)
        py_random.shuffle(players)

        try:
            game = Game(ctx, players, mode_str, speed)
            self.games[ctx.channel.id] = game
            await game.run()
        except Exception as e:
            await ctx.send(_(f"An error happened during the Werewolf. Please try again! {e}"))
            self.games.pop(ctx.channel.id, None)
            raise
...
`Fast`: All major action timers are limited to 45 seconds and number of days to play is dependent on the number of players plus 3 days.
`Blitz`: Warning: This is a faster game speed suitable for experienced players. All action timers are limited to 30 seconds and number of days to play is dependent on the number of players plus 3 days."""
                ),
                colour=self.bot.config.game.primary_colour,
            ).set_author(name=str(ctx.author), icon_url=ctx.author.display_avatar.url)
        )
    ...
    async def myrole(self, ctx):
        ...
        if not isinstance(game, Game):
            return await ctx.send(_("The game state is invalid. Please start a new game."))
        ...
        if player is None:
            return await ctx.send(
                _(
                    "You asked for your role in {channel} but your info couldn't be found."
        ...
        try:
            if player.role != player.initial_roles[0]:
                initial_role_info = _(" A **{initial_roles}** initially").format(
                    initial_roles=", ".join(
                        [game.get_role_name(r) for r in player.initial_roles]
                    )
                )
            else:
                initial_role_info = ""
            ...
            await ctx.author.send(
                _(
                    "Checking your role in {ww_channel}... You are a **{role_name}**!{initial_role_info}\n\n{description}"
                ).format(
            ...
            return await ctx.send(
                _("I couldn't send a DM to you {author}.").format(author=ctx.author.mention)
            )

    @werewolf.command(brief=_("View descriptions of game roles"))
    @locale_doc
    async def roles(self, ctx, *, role: Optional[str] = None):
        _(
            """View the descriptions of roles in the Werewolf game.
            `{prefix}ww roles` to see all roles.
            `{prefix}ww roles <role name here>` to view info about a role.
            """
        )
        ...
        role_groups = [
            {
                ...
                "members": (
                    "Werewolf, White Wolf, Cursed Wolf Father, Big Bad Wolf, Wolf Shaman - "
                    f"{restriction}, Wolf Necromancer - {restriction}"
                ),
            ...
            },
            {
                ...
                "members": (
                    "Villager, Pure Soul, Seer, Witch, Hunter, Healer, Amor, Knight, Sister, Brother, The Old, Fox, Judge, "
                    f"Paragon - {restriction}, Ritualist - {restriction}, Troublemaker - {restriction}, Lawyer - {restriction}, War Veteran - {restriction}"
                ),
                "goal": _("Must find and eliminate the werewolves"),
            },
            {
                "side": _("The Ambiguous"),
                "members": f"Thief, Wild Child, Maid, Wolfhound, Raider - {restriction}",
                "goal": _("Make their side win"),
            },
            {
                "side": _("The Loners"),
                "members": (
                    f"White Wolf - {_('Be the sole survivor')}, Flutist - {_('Must enchant every living inhabitants')}, Superspreader - {_('Infect all the players with your virus')} {restriction}"
                ),
        ...
        if role is None:
            ...
            tip = _(
                "Use `{prefix}ww roles <role>` to view the description on a specific role."
            ).format(prefix=ctx.clean_prefix)
        ...
        role_groups.reverse()
        display_name = search_role.title().replace("_", " ")
