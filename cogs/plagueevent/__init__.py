...
class PlagueOfTheUndying(commands.Cog):
    ...
    async def battle(self, ctx):
        ...
        try:
            ...
            while (
                    ...
                    and datetime.now(timezone.utc) < start + timedelta(minutes=10)
            ):
                if player_turn:
            ...
            if player_stats["hp"] > Decimal('0') and monster_stats["hp"] <= Decimal('0'):
                ...
                if Decimal(str(random.random())) < egg_drop_chance:
                    egg_found = True
                    egg_hatch_time = datetime.now(timezone.utc) + timedelta(
    ...
    def has_acted_recently(self, user_id, action):
        ...
        if (datetime.now(timezone.utc) - last_act_time).total_seconds() < cooldown:
            return True
        return False
    ...
    def get_cooldown_remaining(self, user_id, action):
        ...
        cooldown = cooldown_periods.get(action, 3600)
        elapsed = (datetime.now(timezone.utc) - last_act_time).total_seconds()
    ...
    def set_player_action(self, user_id, action):
        ...
        self.action_cooldowns[user_id][action] = datetime.now(timezone.utc)
