    ...
    async def predicate(ctx):
        async with ctx.bot.pool.acquire() as conn:
            ...
            if last_gift:
                ...
                now = datetime.datetime.now(timezone.utc)  # Explicitly use datetime.datetime.now(timezone.utc)
...
class GiftForward(commands.Cog):
    ...
    async def distribute_gift_by_tier(self, conn, gift):
        tier = gift['tier']
        log_channel = self.bot.get_channel(1404885707788648448)
    ...
    async def giftforward(
        ...
        try:
            # Notify in logs
            log_channel = self.bot.fetch_channel(1404885707788648448)
            if log_channel:
        ...
        async with self.bot.pool.acquire() as conn:
            ...
            for i in range(0, len(gifts), 5):
                ...
                for gift in gifts[i:i + 5]:
                    ...
                    wait_time = datetime.now(timezone.utc) - gift['timestamp']
