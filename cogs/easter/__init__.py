...
class Easter(commands.Cog):
    ...
    async def battle(self, ctx):
        ...
        try:
            ...
            if ctx.author.id in self.player_battle_cooldowns:
                if datetime.now(timezone.utc) < self.player_battle_cooldowns[ctx.author.id]:
                    time_left = self.player_battle_cooldowns[ctx.author.id] - datetime.now(timezone.utc)
            ...
            if cooldown_key in self.team_battle_cooldowns:
                if datetime.now(timezone.utc) < self.team_battle_cooldowns[cooldown_key]:
                    time_left = self.team_battle_cooldowns[cooldown_key] - datetime.now(timezone.utc)
    ...
    async def vote(self, ctx):
        ...
        try:
            ...
            if player_team in self.team_ability_unlocked:
                if datetime.now(timezone.utc) < self.team_ability_unlocked[player_team]:
                    time_left = self.team_ability_unlocked[player_team] - datetime.now(timezone.utc)
                    hours, remainder = divmod(time_left.total_seconds(), 3600)
                    minutes, seconds = divmod(remainder, 60)
                    await self.bot.reset_cooldown(ctx)
                    return await ctx.send(_(
    ...
    async def start_guardian_battle(self, ctx, party_members, attacker_team, defender_team):
        ...
        battle_end_time = datetime.now(timezone.utc) + timedelta(hours=1)
        ...
        self.team_battle_cooldowns[f"{attacker_team}_{defender_team}"] = datetime.now(timezone.utc) + timedelta(minutes=15)
        
        while battle_ongoing and datetime.now(timezone.utc) < start_time + timedelta(minutes=20):
...
class JoinBattleView(discord.ui.View):
    ...
    async def join(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id in self.cog.player_battle_cooldowns:
            if datetime.now(timezone.utc) < self.cog.player_battle_cooldowns[interaction.user.id]:
                time_left = self.cog.player_battle_cooldowns[interaction.user.id] - datetime.now(timezone.utc)
                hours, remainder = divmod(time_left.total_seconds(), 3600)
                minutes, seconds = divmod(remainder, 60)
    ...
    def make_callback(self, ability_name):
        async def callback(interaction: discord.Interaction):
            ...
            if self.team in self.cog.team_ability_unlocked:
                if datetime.now(timezone.utc) < self.cog.team_ability_unlocked[self.team]:
                    time_left = self.cog.team_ability_unlocked[self.team] - datetime.now(timezone.utc)
                    hours, remainder = divmod(time_left.total_seconds(), 3600)
                    minutes, seconds = divmod(remainder, 60)
            ...
            if team_data["upgrades"]["ability_votes"][ability_name] >= 5:
                ...
                self.cog.team_ability_unlocked[self.team] = datetime.now(timezone.utc) + timedelta(hours=24)
            ...
            self.cog.player_vote_cooldowns[interaction.user.id] = datetime.now(timezone.utc) + timedelta(days=1)
