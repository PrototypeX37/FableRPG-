...
class CouplesTowerBattle(TowerBattle):
    """
    Aphrodite issues a challenge for two married players.
    """
    ...
    async def handle_level_30_ceremony(self):
        ...
        embed = discord.Embed(
            title="🌟 THE APEX OF LOVE - DIVINE RECOGNITION 🌟",
            description=f"*{partner1.display_name} and {partner2.display_name} stand before Aphrodite, bathed in celestial light. The Goddess of Love resonates with their pure love...*\n\n"
                       "**Her voice echoes through the heavens:**\n"
                       "*'Beloved ones, you have conquered all of my trials through the power of your eternal bond. Your love has proven unbreakable, pure, and divine. The fates shall choose your blessing...'*",
        ...
        embed.add_field(
            ...
            value=f"🌟 **{partner1.display_name}** - Prestige {partner1_prestige}\n"
                  ...
                  f"🗼 **Challenge Reset** - Return to Level 1 with greater glory",
            inline=False
        )
        ...
        embed.add_field(
            name="💕 **ETERNAL BOND** 💕", 
            value="*Your love story continues beyond Olympus. Until you meet again in front of Aphrodite, "
                  "let your bond be a beacon of hope for all who seek true love.*\n\n"
                  "**The Challegnde of the Love Goddess awaits your return...**",
            inline=False
        )
        
        embed.set_footer(text="Thank you for playing the Love Challenge• Aphrodite will spread your tale on Olympus")
        ...
        try:
            ...
            await self.ctx.send("🌟✨💕 *The Goddess fades into mist, but your love remains eternal* 💕✨🌟")
                
        except Exception as e:
            await self.ctx.send(f"🌟 **Congratulations!** You have completed the Love Challenge! 🌟\n*Error in ceremony: {e}*")
    ...
    async def initialize_forge_heat(self):
        ...
        await self.add_to_log("🔥 **THE FORGE IGNITES!** Blazing heat fills the chamber! Hephaestus whispers: 'Save yourself! Let them burn!' But true love demands sacrifice...")
    ...
    async def process_forge_heat_damage(self):
        ...
        if partner1_shields and partner2_shields:
            ...
            await self.add_to_log(f"😈 **GREED TRIUMPHANT!** Neither partner shields the other! Hephaestus laughs as your selfishness amplifies the forge's punishment!")
        ...
        if deaths:
            await self.add_to_log("💔 **LOVE ENDS IN FLAME** 💔\n*The forge's heat has claimed a life. In the Love Challenge, love cannot survive alone.*")
        ...
        if self.forge_heat_intensity >= 100 and self.forge_heat_intensity < 110:
            ...
            await self.add_to_log("🌋 **INFERNAL INTENSITY!** The forge burns like the very depths of the Underworld! Only the purest love can survive!")
    ...
    async def initialize_despair_system(self):
        ...
        await self.add_to_log("🌫️ **THE VALLEY OF DESPAIR!** Dark mists fill your minds with hopelessness. Oizys whispers: 'Give up... love is meaningless... you'll never succeed...'")
    ...
    async def process_turn_with_jealousy_whispers(self):
        ...
        # Every 3 turns: Narcissus whisper jealous lies
        if self.turn_counter % 3 == 0:
            # Add jealousy whispers to battle log (these are just thematic, not real gameplay effects)
            jealous_whispers = [
                "🪞 *Narcissus whispers:* \"Your partner is doing more damage than you... they think you're weak!\"",
                "🪞 *Narcissus hisses:* \"They're not trying as hard as you are... why should you struggle alone?\"",
                "🪞 *Narcissus taunts:* \"Look how they keep getting hurt... maybe they want to be saved instead of saving!\"",
                "🪞 *Narcissus mocks:* \"Your love isn't equal... one always gives more than the other.\"",
                "🪞 *Narcissus sneers:* \"They're holding back their real power... don't they trust you?\"",
                "🪞 *Narcissus lies:* \"You always have to carry them in battle... when will they carry you?\"",
            ]
    ...
    async def handle_guardian_test(self):
        """Level 20: Harmonia's Test coordination challenge"""
        embed = discord.Embed(
            title="🛡️ Harmonia's Test of Unity 🛡️",
            description="Harmonia halts the battle!\n\n**Both partners must type 'together' within 10 seconds to continue!**",
        ...
        try:
            ...
            if len(responses) >= 2:
                await self.add_to_log("💕 **SUCCESS!** Your unity impresses the Goddess! The battle continues!")
                return True
            else:
                await self.add_to_log("💔 **COORDINATION FAILURE!** The Goddess declares your love unworthy!")
                await self.execute_guardian_punishment()
                await self.ctx.send("💔 **BATTLE FAILED!** Level 20's Goddess's Test requires both partners to type 'together' within 10 seconds. Your love has been found lacking...")
                return False
                
        except asyncio.TimeoutError:
            await self.add_to_log("💔 **TIMEOUT FAILURE!** The Goddess declares your love unworthy!")
            await self.execute_guardian_punishment()
            await self.ctx.send("💔 **BATTLE FAILED!** Level 20's Goddess' Test requires both partners to type 'together' within 10 seconds. Your love has been found lacking...")
            return False
    
    async def execute_guardian_punishment(self):
        """Execute Harmonia's lethal punishment for failing the coordination test"""
        from decimal import Decimal
        
        partners = [c for c in self.player_team.combatants if not c.is_pet]
        
        await self.add_to_log("⚡ **HARMONIA'S WRATH!** The Goddess unleashes devastating divine judgment upon the unworthy lovers!")
        ...
        for partner in partners:
            if partner.is_alive():
                ...
                await self.add_to_log(f"💀 **DIVINE PUNISHMENT!** {partner.name} is struck down by the Goddess' wrath for **{self.format_number(lethal_damage)} HP** true damage!")
        ...
        for pet in [c for c in self.player_team.combatants if c.is_pet]:
            if pet.is_alive():
                ...
                await self.add_to_log(f"💀 **COLLATERAL DAMAGE!** {pet.name} perishes in Harmonia's divine fury!")
    ...
    async def process_turn_with_despair_mechanics(self):
        ...
        if (current_combatant in self.enemy_team.combatants and "Oizys" in current_combatant.name and 
            ...
            if random.random() < 0.30:  # Reduced to 30% to be less spammy
                gained = await self.apply_despair_effects(target, 1, "Oizys whispers", show_message=False)
                total_despair_gained += gained
                despair_whispers = [
                    f"👻 Oizys whispers doubts into {target.name}'s mind...",
    ...
    async def chaos_elemental_special_attack(self, target):
        """Chaos unleashes special chaos effects"""
        chaos_specials = [
            "🌀 Reality Warp",
            "⚡ Dimension Strike", 
            "🌪️ Chaos Vortex",
