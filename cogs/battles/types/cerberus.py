...
class CerberusBattle(Battle):
    """Implementation of Cerberus Challenge battle"""
    
    def __init__(self, ctx, teams, **kwargs):
        super().__init__(ctx, teams, **kwargs)
        
        # Cerberus-specific configurations
        self.cerberus_level = kwargs.get("cerberus_level", 1)
        ...
        if settings_cog:
            ...
            self.config = {
                "allow_pets": settings_cog.get_setting("cerberus", "allow_pets", default=True),
                "class_buffs": settings_cog.get_setting("cerberus", "class_buffs", default=True),
                "element_effects": settings_cog.get_setting("cerberus", "element_effects", default=True),
                "luck_effects": settings_cog.get_setting("cerberus", "luck_effects", default=True),
                "reflection_damage": settings_cog.get_setting("cerberus", "reflection_damage", default=True),
                "fireball_chance": settings_cog.get_setting("cerberus", "fireball_chance", default=0.3),
                "cheat_death": settings_cog.get_setting("cerberus", "cheat_death", default=True),
                "tripping": settings_cog.get_setting("cerberus", "tripping", default=True),
                "status_effects": settings_cog.get_setting("cerberus", "status_effects", default=False),
                "pets_continue_battle": settings_cog.get_setting("cerberus", "pets_continue_battle", default=False)
            }
        else:
        ...
        self.cerberus_stages = {
            # Keep your stages/moves/passives for now; you can retheme later
            "Frostbite Wyrm": {
                "level_range": (1, 5),
                "moves": {
        ...
        # Get the boss team and player team
        self.cerberus_team = teams[0]  # Cerberus is always the first team
        self.player_team = teams[1]    # Players are the second team
        
        # References
        self.cerberus = self.cerberus_team.combatants[0]  # Boss combatant
    ...
    def get_participants(self):
        """Get list of participant user IDs for replay storage (cerberus-specific)"""
        participants = []
    ...
    async def start_battle(self):
        ...
        # Determine which stage we're fighting based on level
        stage_name = self.get_cerberus_stage_name()
        self.cerberus.stage = stage_name
        self.cerberus.name = f"{stage_name} (Level {self.cerberus_level})"
        self.cerberus.passives = self.cerberus_stages[stage_name]["passives"]
        
        # Add the initial message to the battle log
        await self.add_to_log(f"The battle against {self.cerberus.name} has begun! 🐺")
        
        # Add passive effect descriptions
        passive_descriptions = []
        for passive in self.cerberus.passives:
        ...
        if passive_descriptions:
            await self.add_to_log("**Cerberus's Passive Effects:**\n" + "\n".join(passive_descriptions))
            # Ensure action number continues incrementing after passives
            self.current_turn = 1  # Set to 1 to start with boss's turn
    ...
    async def process_turn(self):
        ...
        if current_combatant == self.cerberus:
            # Cerberus's turn
            await self.process_cerberus_turn(current_combatant)
        else:
            # Player's turn
        ...
        if hasattr(self.ctx.bot.cogs["Battles"], "battle_factory"):
            ...
            for player in self.player_team.combatants:
                if player.is_pet and player.is_alive():
                    ...
                    setattr(player, 'enemy_team', self.cerberus_team)
    ...
    async def process_cerberus_turn(self, cerberus):
        """Process Cerberus's turn"""
        # Get stage and available moves
        stage_name = cerberus.stage
        stage_info = self.cerberus_stages.get(stage_name, self.cerberus_stages["Frostbite Wyrm"])
        ...
        if not targets:
            await self.add_to_log(f"{cerberus.name} has no valid targets!")
            return
            
        # Apply the move to targets
        base_damage = move_info["dmg"] * (1 + (0.1 * (self.cerberus_level - 1)))
        ...
        for target in targets:
            ...
            if self.config.get("element_effects", True) and hasattr(self.ctx.bot.cogs["Battles"], "element_ext"):
                # Boss is currently treated as Water element (change later if wanted)
                cerb_element = "Water"
                ...
                if target_element:
                    ...
                    try:
                        element_modifier = await self.ctx.bot.cogs["Battles"].element_ext.get_element_multiplier(cerb_element, target_element)
                        
                        # Element messages will be added to the attack message
                        if element_modifier > 1.0:
                            element_message = f" ({cerb_element} is strong against {target_element}!)"
                        elif element_modifier < 1.0:
                            element_message = f" ({cerb_element} is weak against {target_element}!)"
                    except Exception:
                        # Fallback if element extension fails
                        element_modifier = 1.0
            
            # Calculate damage using direct armor subtraction
            ...
            if (target.is_pet and hasattr(self.ctx.bot.cogs["Battles"], "battle_factory")):
                pet_ext = self.ctx.bot.cogs["Battles"].battle_factory.pet_ext
                damage, defender_messages = pet_ext.process_skill_effects_on_damage_taken(target, cerberus, damage)
            
            # Apply Death's Embrace passive effect (10% chance to instantly kill)
            death_embrace_triggered = False
            if "Death's Embrace" in cerberus.passives and random.random() < 0.1:
            ...
            if not cheat_death_triggered and not death_embrace_triggered:
                ...
                if "Soul Devourer" in cerberus.passives:
                    if isinstance(damage, Decimal):
                        stolen_health = damage * Decimal('0.15')  # 15% of damage
                    else:
                        stolen_health = float(damage) * 0.15
                    cerberus.heal(stolen_health)
            
            # Log the action
            if death_embrace_triggered:
                message = f"{cerberus.name} uses **{move_name}** on {target.name}! "
            ...
            if "Soul Devourer" in cerberus.passives and not death_embrace_triggered:
                if isinstance(damage, Decimal):
                    stolen_health = damage * Decimal('0.15')
                else:
                    stolen_health = float(damage) * 0.15
                message += f"\n👻 **SOUL DEVOURER!** {cerberus.name} steals **{self.format_number(stolen_health)} HP** from {target.name}!"
            ...
            if (self.config.get("class_buffs", True) and 
                ...
                if reflection_multiplier > 0:
                    ...
                    if reflected_damage > 0:
                        cerberus.take_damage(reflected_damage)
                        message += f"\n🛡️ {target.name}'s shield reflects **{self.format_number(reflected_damage)} HP** damage back to {cerberus.name}!"
    ...
    async def process_player_turn(self, player):
            ...
            if player_element:
                ...
                if element_mod != 0:
                    ...
                    if element_multiplier > 1.0:
                        element_message = f" ({player_element} is strong against {cerb_element}!)"
                    else:
                        element_message = f" ({player_element} is weak against {cerb_element}!)"
        
        # Use standard damage calculation system
        # Check for special damage types
        ignore_armor = getattr(cerberus, 'ignore_armor_this_hit', False)
        true_damage = getattr(cerberus, 'true_damage', False)
        bypass_defenses = getattr(cerberus, 'bypass_defenses', False)
        ignore_all = getattr(cerberus, 'ignore_all_defenses', False)
        
        if ignore_all or true_damage or ignore_armor or bypass_defenses:
            final_damage = damage  # No armor reduction
            blocked_damage = Decimal('0')
        else:
            blocked_damage = min(damage, cerberus.armor)
            final_damage = max(damage - cerberus.armor, Decimal('10'))
        
        # Clear special damage flags
        for flag in ['ignore_armor_this_hit', 'true_damage', 'bypass_defenses', 'ignore_all_defenses']:
            if hasattr(cerberus, flag):
                delattr(cerberus, flag)
        
        # Apply damage reduction from passive effects AFTER standard damage calculation
        damage_reduction = Decimal('0.0')
        
        # Apply passive effects
        if "Ice Armor" in cerberus.passives:
        ...
        if (player.is_pet and hasattr(self.ctx.bot.cogs["Battles"], "battle_factory")):
            pet_ext = self.ctx.bot.cogs["Battles"].battle_factory.pet_ext
            final_damage, skill_messages = pet_ext.process_skill_effects_on_attack(player, cerberus, final_damage)
        ...
        if "Reality Bender" in cerberus.passives and random.random() < 0.5:
            ...
            cerberus.take_damage(final_damage)
            message = f"{player.name} attacks!{crit_message} {cerberus.name} takes **{self.format_number(final_damage)} HP** damage.{element_message}"
        ...
        if (self.config.get("class_buffs", True) and 
            ...
            if random.random() < fireball_chance:
                ...
                cerberus.take_damage(fireball_damage)
                message += f"\n🔥 **FIREBALL!** {player.name} casts a fireball for **{self.format_number(fireball_damage)} HP** additional damage!"
            
        # Handle boss's reflection damage if reflection_damage is enabled
        if self.config.get("reflection_damage", True) and "Reflective Scales" in cerberus.passives:
            ...
            if reflection_damage > 0:
                ...
                message += f"\n{cerberus.name}'s reflective scales return **{self.format_number(reflection_damage)} HP** damage!"
        ...
        if (self.config.get("class_buffs", True) and 
            ...
            if reflection_multiplier > 0:
                ...
                if reflection_damage > 0:
                    cerberus.take_damage(reflection_damage)
        ...
        if hasattr(player, 'summon_skeleton'):
            ...
            # Add skeleton to player team (cerberus battles use player_team and cerberus_team)
            if player in self.player_team.combatants:
                self.player_team.combatants.append(skeleton)
                message += f"\n💀 A skeleton warrior joins your side!"
            else:
                self.cerberus_team.combatants.append(skeleton)
                message += f"\n💀 A skeleton warrior joins the cerberus side!"
    ...
    async def create_battle_embed(self):
        ...
        stage_name = self.cerberus.stage
        ...
        # Create embed
        embed = discord.Embed(
            title=f"Cerberus Challenge - Level {self.cerberus_level}",
            description=f"The **{stage_name}** is battling the party...",
            color=discord.Color.blue()
        )
        
        # Add cerberus info
        cerb_hp = max(0, float(self.cerberus.hp))
        cerb_max_hp = float(self.cerberus.max_hp)
        cerb_hp_percent = (cerb_hp / cerb_max_hp) * 100 if cerb_max_hp > 0 else 0
        cerb_hp_bar = self.create_hp_bar(cerb_hp, cerb_max_hp, length=20)
        
        # Get boss element emoji (currently Water)
        cerb_element_emoji = element_to_emoji.get("Water", "❓")
        
        embed.add_field(
            name=f"🐺 {self.cerberus.name} {cerb_element_emoji}",
            value=f"HP: {cerb_hp:.1f}/{cerb_max_hp:.1f} ({cerb_hp_percent:.1f}%)\n{cerb_hp_bar}",
            inline=False
        )
    ...
    async def end_battle(self):
        ...
        if self.cerberus.hp <= 0:
            # Players win
            await self.add_to_log(f"**Victory!** The {self.cerberus.name} has been defeated!")
            await self.update_display()
            return True
        ...
        if all(player.hp <= 0 for player in self.players):
            # Boss wins
            await self.add_to_log(f"**Defeat!** The party has been wiped out by the {self.cerberus.name}!")
            await self.update_display()
            return False
    ...
    def get_next_combatant(self):
        ...
        # Simplified turn system - alternate between boss and a random player
        # Use current_turn instead of action_number to determine whose turn it is
        if self.current_turn % 2 == 0:  # Boss on even turns (0, 2, 4, etc.)
            self.current_turn += 1
            ...
            return self.cerberus  # Fallback
    
    def get_alive_players(self):
        """Get all alive players"""
        return [player for player in self.players if player.hp > 0]
        
    def get_cerberus_stage_name(self):
        """Determine which stage to use based on level"""
        for stage_name, stage_info in self.cerberus_stages.items():
            level_range = stage_info.get("level_range", (1, 5))
            if level_range[0] <= self.cerberus_level <= level_range[1]:
                return stage_name
