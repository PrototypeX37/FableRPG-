...
class DragonExtension:
    ...
    async def _get_abilities_map(self, ability_type: str) -> Dict[str, Dict[str, Any]]:
            ...
            for stage_info in self.default_dragon_stages.values():
                for move_name, move_info in stage_info.get("moves", {}).items():
                    ability_map[move_name] = {
                        "description": (
                            f"Effect: {move_info['effect']}, "
                            f"Damage: {move_info['dmg']}, "
                            f"Chance: {int(move_info['chance'] * 100)}%"
                        ),
                        "dmg": move_info["dmg"],
    ...
    async def get_passive_descriptions(self, passive_names: List[str]) -> Dict[str, str]:
        """Get passive descriptions for the provided names."""
        descriptions = {}
        try:
            async with self.bot.pool.acquire() as conn:
                rows = await conn.fetch(
                    "SELECT name, description FROM ice_dragon_abilities "
                    "WHERE ability_type = 'passive' AND name = ANY($1::text[])",
                    passive_names,
                )
            for row in rows:
    ...
    async def get_dragon_stage(self, level: int) -> Dict[str, Any]:
        ...
        try:
            ...
            move_map = await self._get_abilities_map("move")
            ...
            for stage_name, stage_info in self.default_dragon_stages.items():
                ...
                if min_level <= level <= max_level:
                    return {"name": stage_name, "info": stage_info}

        return {"name": "Eternal Frost", "info": self.default_dragon_stages["Eternal Frost"]}
    ...
    async def update_dragon_progress(self, bot, dragon_level: int, weekly_defeats: int, victory: bool = True) -> dict:
        ...
        async with bot.pool.acquire() as conn:
            exists = await conn.fetchval('SELECT 1 FROM dragon_progress WHERE id = 1')
